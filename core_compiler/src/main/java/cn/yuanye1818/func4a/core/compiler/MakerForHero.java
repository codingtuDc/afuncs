...
public class MakerForHero extends CoreMaker {
    ...
    private ClassBuilder getClass(CE ce, String heroName) {
        ...
        if (Utils.containsClass(cs, heroName)) {
            ...
            getAct.addCodeLine("Object binder = this.binder;");
            getAct.addCodeLine("if (binder instanceof $T) {", activityClass);
            getAct.addCodeLine("  return ($T) binder;", activityClass);
            getAct.addCodeLine("} else if (binder instanceof $T) {", fragmentClass);
            getAct.addCodeLine("  return (($T) binder).getActivity();", fragmentClass);
            getAct.addCodeLine("}");
            getAct.addCodeLine("return null;");
